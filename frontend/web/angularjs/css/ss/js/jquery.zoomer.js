...
//            if(IFrameHeight*zoom > BoxHeight){
//                scroll = true;
//            }
            options.scroll = false;
            if($(this).contents().height()*options.zoom > options.height){

                options.width =  options.width-options.scrollWidth;
                options.scroll = true;
            }
            console.log("scroll:" +options.scroll);

            $el
                .css(invisible)
                .css(unselectable)
            ;
//            options.width = $(this).contents().find("body").width()*options.zoom;
//            console.log('86 = ' + $(this).contents().find("body").width());
//            console.log('87 = ' + options.width);
        ...
        updateWrapper: function() {
            ...
            $.each([options.zoomerWrapper.get(0), options.zoomerCover.get(0), options.zoomerLoader.get(0), options.zoomerSmall.get(0)], function(){
                $(this).css({
                    height: options.height,
                    width: options.scroll ? options.width+options.scrollWidth : options.width
                });
            });
                ...
                .load(function(){
                    $el.contents().find('html').css({
                        ...
                        'height':options.height/ options.zoom,
                          width: options.width/ options.zoom,
                            'overflow-x':'hidden',
                            'overflow-y': options.scroll? 'auto' : 'hidden'
