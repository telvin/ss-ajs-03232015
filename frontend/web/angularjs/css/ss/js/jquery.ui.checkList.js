(function($) {
	$.widget("ui.checkList", {	
		...
		_create: function() {
            ...
            //render outer div to enclose table (for body scrolling)
            var pEnclose = $('<div/>').addClass('scroll-pane');

			container.append(toolbar);

            pEnclose.append(o.objTable);
            container.append(pEnclose);
			//container.append(o.objTable);

//            pEnclose.append(container);
//            el.append(pEnclose);
			el.append(container);
		...
		_addItem: function(Item){
			...
			var itemId = Item.prefix_id + (o.icount);// generate item id
            o.icount++;
			var itm = $('<tr/>');
			var chk = $('<input/>').attr('type','checkbox').attr('id',itemId)
                    ...
                    .attr('name',Item.name)
                    .attr('checked', Item.checked=='1'?true:false);
			
			itm.append($('<td/>').append(chk));
			var label = $('<label/>').attr('for',itemId).text(Item.text);
