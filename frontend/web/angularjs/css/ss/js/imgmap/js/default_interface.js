...
function gui_row_select(id, setfocus, multiple) {
	...
	props[id].style.background = '#444444';
}
...
function gui_input_change(e) {
	...
	if (obj.name == 'img_href')        {myimgmap.areas[id].ahref   = obj.value;}
        ...
        console.log('gui_input_change', id, myimgmap.areas[id].atarget);
        var elements = [];
        elements = getElementsByClassName('open-custom-tip', props[id]);

        if(myimgmap.areas[id].atarget == '_popup'){
            for(i in elements ){
            ...
            //getElementsByClassName('open-custom-tip').value;
        }else{
            for(i in elements ){
                if (elements.hasOwnProperty(i)) { //check if the element is not a prototype
                    elements[i].style.display = "none";
                }
            }
        }
    }
    else if (obj.name == 'img_custom_tip') {myimgmap.areas[id].acustom_tip = obj.value;}
...
/**
 *	Called from imgmap when the new canvas is added and before gui_addArea
 */
function gui_done_draw(evtData) {
    //example of evtData: ({id:1, obj:{aid:1, shape:"rect", ahref:"", atitle:"", aalt:"", atarget:"", label:{}, alt:"#1 ", lastInput:"821,338,1334,629"}})
    //console.log(evtData.obj.lastInput);
    //console.log(myimgmap.areas.toSource());
    //console.log(evtData.toSource()); //.toSource() debug object by firefox
}

/**
 *	Called from imgmap when a new area is added.
 */
function gui_addArea(id, params) {
	//var id = props.length;
	//id = 1;
	...
	props[id].innerHTML = createRowByTemplate(id, params);
    ...
    /**
     * The additional script when create new area  in addNewArea function
     * add new attribute area_order to the div that displaying the infomation of area
     * everytime when create a new area, reorder those divs by area_order
     */
    $("#"+props[id].id).attr("area_order",id);
    var list = $('#form_container');
    var listItems = list.find('div').sort(function(a,b){ return $(a).attr('area_order') - $(b).attr('area_order'); });
    list.find('div').remove();
    list.append(listItems);
}

function createRowByTemplate(id, params){
    var href="", alt="", coords="", target="", shape="", custom_tip="";
    if(!$.isEmptyObject(params)){
        href = typeof params.href === "undefined" ? "" : params.href;
        alt = typeof params.alt === "undefined" ? "" : params.alt;
        coords = typeof params.coords === "undefined" ? "" : params.coords;
        target = typeof params.target === "undefined" ? "" : params.target;
        custom_tip = typeof params.custom_tip === "undefined" ? "" : params.custom_tip;
        shape = typeof params.shape === "undefined" ? "" : params.shape;
    }

    var temp = '<input type="hidden" name="img_id" class="img_id" value="' + id + '"/>';
    temp += '<span class="area-number">'+(id+1)+'.</span>';
    ...
    temp+= '<select name="img_shape" class="img_shape" hidden="hidden">';
//    temp+= '<option value="rect">rectangle</option>';
//
//    if (document.getElementById('dd_output').value != 'css') {
//        temp+= '<option value="circle" '+ddlDefaultVal("circle", shape)+'>circle</option>';
        temp+= '<option selected="selected" value="poly" '+ddlDefaultVal("poly", shape)+'>polygon</option>';
//        temp+= '<option value="bezier1" '+ddlDefaultVal("bezier1", shape)+'>bezier</option>';
//    }
    temp+= '</select>';
    temp+= '<input type="text" name="img_coords" class="img_coords" style="display:none" value="'+coords+'">';
    temp+= 'Link: <input type="text" name="img_href" class="img_href" value="'+href+'">';
    temp+= 'Tooltip: <input type="text" name="img_alt" class="img_alt" value="'+alt+'">';
    temp+= 'Target: <select name="img_target" class="img_target">';
//    temp+= '<option value=""  >&lt;not set&gt;</option>';
    temp+= '<option value="_self" '+ddlDefaultVal("_self", target)+'>open in same window</option>';
    temp+= '<option value="_blank" '+ddlDefaultVal("_blank", target)+'>open in new tab</option>';
    temp+= '<option value="_popup" '+ddlDefaultVal("_popup", target)+'>open in the popup</option>';
//    temp+= '<option value="_top" '+ddlDefaultVal("_top", target)+'>top window</option>';
    temp+= '</select>';

    temp+= '<a class="open-custom-tip" style="display: none"><span>Popup Content</span></a><textarea name="img_custom_tip" class="img_custom_tip" hidden="hidden">'+custom_tip+'</textarea>'
    return temp;
}

function ddlDefaultVal(value,compare){
...
function gui_removeArea(id) {
	if (props[id]) {
		...
		try {
			...
			//alert('noparent');
            console.log('gui_removeArea exception', err);
		}
	}
}
...
function gui_areaChanged(area) {
	...
	if (props[id]) {
        ...
        if (area.acustom_tip) {props[id].getElementsByTagName('textarea')[0].value = area.acustom_tip;}

        //console.log('gui_areaChanged', JSON.stringify(myimgmap.areas[id]));
        var elements = [];
        elements = getElementsByClassName('open-custom-tip', props[id]);
        if(myimgmap.areas[id].atarget == '_popup'){
            for(i in elements ){
...
function gui_zoom(isFullScreen) {
...
//        pic_container.setAttribute('style','width:'+(pic.oldwidth *scale + 17)+'px;height:'+(pic.oldheight*scale+17)+'px');
//        console.log(pic.oldwidth*scale);
//        console.log(pic.oldheight*scale);
        $("#pic_container").width(pic.oldwidth*scale+20+'px');
        $("#pic_container").height(pic.oldheight*scale+'px');
        $("#pic_container").css({'overflow': 'hidden'});
    }
...
function gui_outputChanged() {
	...
	if (output == 'css') {
		...
		if (clipboard_enabled) {
			...
			temp+= '<img src="'+yii.urls.imgmapPluginPath+'/images/clipboard.gif" onclick="gui_toClipBoard()" style="float: right; margin: 4px; cursor: pointer;"/>';
		}
...
function gui_db_click_area(area){
    duplicate_area(myimgmap, area);
}
...
custom_callbacks : {
	...
	'onAddArea'       : function(evtData)  {gui_addArea(evtData.id, evtData.params);},//to add new form element on gui
    ...
    'onRelaxArea'     : function(evtData){gui_done_draw(evtData);}, // after done drawing
    'onDblClickArea'     : function(obj){gui_db_click_area(obj);} // after done drawing
},
...
props = [];
//imgroot = 'samples/';
imgroot = yii.urls.imgmapPluginPath+'/images/';
outputmode = 'imgmap';
gui_outputChanged();
...
/*
* example of format of item
* {
     id: '0',
     href: '',
     alt: '',
     target: '_self',
     coords: '58,38,116,86,170,48,170,48',
     shape: 'poly'
 }

 * */

function duplicate_area(myimgmap, area){
    total = myimgmap.areas.length;
    //area.aid - index of area
    var copy_item = {
        id: total, //because index ran from 0
        href: area.ahref,
        alt: area.aalt,
        target: area.atarget,
        shape: area.shape,
        coords: area.lastInput,
        custom_tip: area.acustom_tip
    };

    //if last row is empty, remove it and add new row with copy_item (like an update)
    //var lastArea = myimgmap.areas[total-1]; //check on storage data

    /* check on field*/
    lastArea = {
        aid: parseInt($('[id^=img_area_]').eq(total).find('.img_id').val()),
        lastInput: $('[id^=img_area_]').eq(total).find('.img_coords').val(),
        href: $('[id^=img_area_]').eq(total).find('.img_href').val(),
        alt: $('[id^=img_area_]').eq(total).find('.img_alt').val(),
        target: $('[id^=img_area_]').eq(total).find('.img_target').val(),
        shape: $('[id^=img_area_]').eq(total).find('.img_shape').val(),
        custom_tip: $('[id^=img_area_]').eq(total).find('.img_custom_tip').val()

    };

    if(is_empty_area(lastArea)){
        console.log('dbclick');
        myimgmap.removeArea(lastArea.aid);
        copy_item.id = lastArea.aid; //copy_item will replace the removed area
    }


    new_dup_area = myimgmap.addNewArea();
    myimgmap.initArea(copy_item.id,copy_item.shape,copy_item);
    myimgmap._recalculate(copy_item.id, copy_item.coords);//contains repaint
...
function is_empty_area(area){
    //console.log(JSON.stringify(area));
    return ((typeof area.lastInput == 'undefined' || area.lastInput == '') &&
        (typeof area.ahref == 'undefined' || area.ahref == '') &&
        (typeof area.aalt == 'undefined' || area.aalt == '')
        );
