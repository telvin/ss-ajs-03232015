...
function imgmap(config) {
    ...
    this.config.CL_DRAW_BG         = 'transparent';
    this.config.CL_NORM_BOX        = '#E32636';
    this.config.CL_NORM_SHAPE      = '#d00';
    this.config.CL_NORM_BG         = 'transparent';
...
imgmap.prototype.loadImage = function(img, imgw, imgh) {
    //_addNewArea = typeof enableAddNewArea === 'undefined' ? true : false;
    //test for container
...
imgmap.prototype.useImage = function(img) {
    ...
    if (!this._getLastArea()) {
        //init with one new area if there was none editable
        if (this.config.mode != "editor2") {console.log('log-690');this.addNewArea();}
    }
 ...
 *	@see	#initArea
 */
imgmap.prototype.addNewArea = function(params, singleCall) {
    ...
    if(singleCall){
        /**
         * Create new area by clicking on + button:
         * Reorder elements in areas array by id
         * find the missing element then return the index for the next area that about to create
         * if there's no missing element, then find the last area and generate the id normally
         *  */
        var repair_area = Array();
        jQuery.each(this.areas, function(index, item) {
            if(item != null) {
                id =  item.id;
                var item_index = id.substring(id.length,id.length-1);
                repair_area[item_index] = item;
            }
        });
        jQuery.each(repair_area, function(index, item) {
            if(typeof item == "undefined"){
                id = index;
                return false;
            }
        });
        this.areas = repair_area;
        if(id.length >1){
            id = (lastarea) ? parseInt(lastarea.aid) + 1 : 0;
        }
    }else{
        //if(lastarea) console.log(typeof lastarea.aid, lastarea.aid);
        id = (lastarea) ? this.areas[parseInt(lastarea.aid)].aid + 1 : 0;
    }
    //console.log(this.areas);
    ...
    this.currentid = id;
    this.fireEvent('onAddArea', {id: id, params: params});
    return id;
};
 ...
 */
imgmap.prototype.initArea = function(id, shape, params) {
    var href="", alt="", coords="", target="", custom_tip="";
    ...
    //console.log('wth',JSON.stringify(this.areas));
    ...
    this.areas[id].aalt      = alt;
    this.areas[id].atarget   = target; // '_self';
    this.areas[id].acustom_tip   = custom_tip;
...
imgmap.prototype.relaxArea = function(id) {
    ...
    this.fireEvent('onRelaxArea', {id:id, obj:this.areas[id]});
...
//		console.log(this.areas[id].label);
    ...
    this.areas[id] = null;
//    this.areas.splice(id, 1); // remove obj from an array by index
    //_.without(arr, _.findWhere(this.areas, {id: 3})); //remove obj from an array of objs by condition <use underscore.js>
...
imgmap.prototype._putlabel = function(id) {
    ...
    try {
        if (!this.config.label) {
            ...
            label = label.replace(/%n/g, String(parseInt(id)+1));
...
imgmap.prototype._puthint = function(id) {
    try {
        if (!this.config.hint) {
            ...
            hint = hint.replace(/%n/g, String(parseInt(id)+1));
...
imgmap.prototype._updatecoords = function(id) {
    //console.log(JSON.stringify(this.areas[id]));
    console.log('_updatecoords');
...
imgmap.prototype._recalculate = function(id, coords) {
    try {
        if (/\S/.test(coords)) { // string is not empty and not just whitespace
            coords = this._normCoords(coords, this.areas[id].shape, 'preserve');
        }
        else {
//            if(typeof this.areas[id] !== 'undefined')
...
imgmap.prototype.img_mousemove = function(e) {
    ...
    //console.log('img_mousemove1', JSON.stringify(myimgmap.areas[1]));
    ...
    //console.log('img_mousemove2', JSON.stringify(myimgmap.areas[1]));
...
imgmap.prototype.img_mousedown = function(e) {
    ...
    if (this.is_drawing == this.DM_POLYGON_DRAW || this.is_drawing == this.DM_BEZIER_DRAW) {
        ...
        if (this.areas[this.currentid] == this._getLastArea()) {
            ...
            if (this.config.mode != "editor2") {console.log('log-2203');this.addNewArea();}
            return;
        }
        return;
    }

    var target = this.areas[this.currentid].atarget;

    if (this.config.mode == "editor2") {
        if (!this.nextShape) {return;}
        console.log('log-2212');
        this.addNewArea();
        ...
        if (!shape) {shape = 'rect';}
        //console.log(this.currentid,JSON.stringify(this.areas[this.currentid]));
    ...
    this.areas[this.currentid].atarget = target;
...
imgmap.prototype.area_mousemove = function(e) {
    ...
    if (!this.is_drawing) {
        ...
        <!-- custom -->
        if (resizable && xdiff < 6 && ydiff > parseInt(obj.style.height, 10)-6) {
            //move bottom
            obj.style.cursor = 'sw-resize'; //bottom left
        }
        else if (resizable && xdiff > parseInt(obj.style.width, 10) -6 && ydiff > parseInt(obj.style.height, 10)-6) {
            //move bottom
            obj.style.cursor = 'se-resize'; //bottom right
        }
        else if (resizable && Math.abs(xdiff - parseInt(obj.style.width, 10)) < 6 && ydiff < 6) {
            //move bottom
            obj.style.cursor = 'ne-resize'; //top right
        }
        else if (resizable && xdiff < 6 && ydiff < 6) {
            //move bottom
            obj.style.cursor = 'nw-resize'; //top left
        }
...
//        console.log('w:' + parseInt(obj.style.width, 10).toString(), 'h:'+parseInt(obj.style.height, 10).toString());
