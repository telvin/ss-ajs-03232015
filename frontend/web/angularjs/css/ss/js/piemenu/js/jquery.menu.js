(function( $ ) {
    ...
    $.fn.PieMenu = function(options) {
        ...
        var settings = $.extend( {
            ...
            'rotateIconHolder': true,

            'childStartPosLeft': 0, //this couple help we align child items should have to be behind main button
            'childStartPosTop': 0
        }, options);
        ...
        function setPosition(val){
            $(settings.menu_element).each(function(i,ele){
                $(ele).css({
                    'left' : (val==0)?settings.childStartPosLeft:y_pos[i],
                    'top' : (val==0)?settings.childStartPosTop:-x_pos[i]
                });
            });
        }
        ...
        var clickHandler = function() {
            ...
            if(settings.rotateIconHolder)
                $(this).toggleClass("btn-rotate");
        };

        $(settings.menu_button).bind('click', clickHandler);

        //start position for child items
        $(settings.menu_element).each(function(i,ele){
            $(ele).css({
                'left' : settings.childStartPosLeft,
                'top' : settings.childStartPosTop
            });
        });
